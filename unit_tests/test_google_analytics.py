from digital_street_3d_ui.custom_extensions.google_analytics.main import build_form_errors
...
class TestGoogleAnalytics(unittest.TestCase):
    ...
    def render_form(self):
        ...
        html = render_template('app/layout.html', form=form).replace('\n', '')

        return html

    def test_raised_form_errors_appear_in_script_block(self):
        with mock.patch.dict(app.config, {'GOOGLE_ANALYTICS_KEY': '123'}):
            html = self.render_form()

        # Check that the script block is there
        self.assertIn('<script type="application/json" id="form-error-data" data-form-name="ExampleForm">', html)
        ...
        self.assertEqual(error_json,  [{'name': 'string_field', 'errors': ['StringField is required']}])

    def test_raised_form_errors_dont_appear_in_script_block_when_analytics_disabled(self):
        with mock.patch.dict(app.config, {'GOOGLE_ANALYTICS_KEY': False}):
            html = self.render_form()

        # Check that the script block is not there
        self.assertNotIn('<script type="application/json" id="form-error-data" data-form-name="ExampleForm">', html)
...
class TestGoogleAnalyticsFormErrorBuilder(unittest.TestCase):
    ...
    def test_build_form_errors_with_nested_sub_forms(self):
        ...
        self.assertIsInstance(result, list)
        expected_error_list = [
            {'errors': ['Another field is required'], 'name': 'subform-sub_subform-another_field'},
